...
class DocumentProduce:
    ...
    def run(self):
        ...
        #             self.pool.submit(lambda p: self.__request_and_write_document(*p),
        #                              [__request_body, __title, __account])
        #
...
def account_manager(force_update):
    ...
    while (
        ...
        if len(account_big_json_dict.get("accounts")) == 0:
            print(
                "已经没有可用的账号，程序退出，日志已生成于 {} 下，账号详情查看 {}".format("/data/log/runtime.log", "/data/conf/account.json"))
            sys.exit()
        else:
            pop_account.append(current_use_account)
            current_use_account = account_big_json_dict.get("accounts").pop()
